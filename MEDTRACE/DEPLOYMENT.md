...
1. Create a PostgreSQL database in Neon or Supabase and copy its connection
   string.
...
3. In Render, set these environment variables:
   ...
   | Variable | Value | Why |
   | --- | --- | --- |
   | `DATABASE_URL` | The managed PostgreSQL connection string | Stores durable shared records. Both `postgres://` and `postgresql://` are supported. |
